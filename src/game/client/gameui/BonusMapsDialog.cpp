...
public:
	CBonusMapPanel( PanelListPanel *parent, const char *name, int bonusMapListItemID ) : BaseClass( parent, name )
	{
		...
		LoadControlSettings( "resource/BonusMapPanel2.res" );
...
CBonusMapsDialog::CBonusMapsDialog(vgui::Panel *parent) : BaseClass(parent, "BonusMapsDialog")
{
	...
	m_pLoadBonusMapButton = new vgui::Button( this, "loadbonusmap", "" );
	m_pLoadBonusMapButton->SetEnabled( false );
	
	m_pMultiplayerSettingsButton = new vgui::Button( this, "OpenMultiplayerOptions", "" );
	m_pMultiplayerSettingsButton->SetCommand( "OpenMultiplayerOptions" );
	
	m_pCancelButton = new vgui::Button( this, "Cancel", "#GameUI_Cancel" );
	m_pCancelButton->SetCommand( "Close" );

	BuildMapsList();
		
	m_pLoadBonusMapButton->SetTabPosition(2);
	m_pMultiplayerSettingsButton->SetTabPosition(3);
	m_pCancelButton->SetTabPosition(4);	
	
	//Setup default multiplayer settings
	...
	LoadControlSettings("resource/BonusMapsDialog2.res");

	m_MultiplayerSettingsDialog = NULL;

	//This won't work in portalcoop :(
#if 0
...
void CBonusMapsDialog::RefreshDialog( BonusMapDescription_t *pMap )
{
	...
	else
	{
		...
		m_pLoadBonusMapButton->SetEnabled( true );
	}
...
void CBonusMapsDialog::OnCommand( const char *command )
{
		...
		if ( BonusMapsDatabase()->IsValidIndex( mapIndex ) )
		{
			...
			if ( shortName && shortName[ 0 ] )
			{
				...
				else
				{
					...
					Q_snprintf( szPlayers, sizeof( szPlayers ), "maxplayers %i\n", pBonusMap->iPlayers );
										
					Q_snprintf( szLanServer, sizeof( szLanServer ), "sv_lan %i\n", m_MultiplayerOptions.bLanServer );
					
					Q_snprintf( szHostname, sizeof( szHostname ), "hostname %s\n", m_MultiplayerOptions.sHostname );
					
					Q_snprintf( szPassword, sizeof( szPassword ), "sv_password %s\n", m_MultiplayerOptions.sPassword );
					...
					if (pSelectedBonusMapPanel)
					{
						...
						ChallengeDescription_t Map = BonusMapsDatabase()->m_CurrentChallengeObjectives;
					
						

						Q_snprintf( szBronze, sizeof( szBronze ), "sv_bonus_score_objective_bronze %i\n", Map.iBronze );
					
						Q_snprintf( szSilver, sizeof( szSilver ), "sv_bonus_score_objective_silver %i\n", Map.iSilver );
					
						Q_snprintf( szGold, sizeof( szGold ), "sv_bonus_score_objective_gold %i\n", Map.iGold );
						
						engine->ClientCmd_Unrestricted( szBronze );					
						engine->ClientCmd_Unrestricted( szSilver );					
						engine->ClientCmd_Unrestricted( szGold );
					}

					engine->ClientCmd_Unrestricted( szLanServer );
					engine->ClientCmd_Unrestricted( szHostname );
					engine->ClientCmd_Unrestricted( szPassword );
					engine->ClientCmd_Unrestricted( szPlayers ); // FIXME: maxplayers won't change while a map is currently running!
					engine->ClientCmd_Unrestricted( sz );
	...
	else if ( !stricmp ( command, "OpenMultiplayerOptions" ) )
	{
		if ( !m_MultiplayerSettingsDialog.Get() )
		{
			m_MultiplayerSettingsDialog = new CBonusMapsMultiplayerSettingsDialog(GetParent());
			VPANEL GameUiDll = enginevgui->GetPanel(PANEL_GAMEUIDLL);
			m_MultiplayerSettingsDialog->SetParent(GameUiDll);
		}
		m_MultiplayerSettingsDialog->Activate();
	}
	else
	{
...
void CBonusMapsDialog::Close()
{
	if (m_MultiplayerSettingsDialog.Get())
		m_MultiplayerSettingsDialog->Close();
...
void CBonusMapsDialog::OnKeyCodePressed( vgui::KeyCode code )
{
	...
	// I wish portalcoop could support the steam controller :(
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CBonusMapsMultiplayerSettingsDialog::CBonusMapsMultiplayerSettingsDialog( vgui::Panel *parent ) : BaseClass( parent, "BonusMapsMultiplayerSettingsDialog" )
{
	SetTitle("#GameUI_BonusMaps_Multiplayer_Options", true);
	SetSizeable(false);
	MoveToCenterOfScreen();
	
	m_MultiplayerOptions = g_pBonusMapsDialog->m_MultiplayerOptions;

	m_pForceRespawnCheck = new vgui::CheckButton(this, "ForceRespawnCheck", "Force Respawn");
	m_pForceRespawnCheck->SetSelected( m_MultiplayerOptions.bForceRespawn );
	
	m_pLanServerCheck = new vgui::CheckButton(this, "LanServerCheck", "Lan Server");
	m_pLanServerCheck->SetSelected(m_MultiplayerOptions.bLanServer);
	
	m_pHostnameTextEntry = new vgui::TextEntry( this, "HostNameTextEntry" );
	m_pHostnameTextEntry->SetText(m_MultiplayerOptions.sHostname);

	m_pPasswordTextEntry = new vgui::TextEntry(this, "PasswordTextEntry");
	m_pPasswordTextEntry->SetText(m_MultiplayerOptions.sPassword);
	
	LoadControlSettings( "resource/BonusMapMultiplayerSettingsDialog.res" );
}

//-----------------------------------------------------------------------------
// Purpose: Destructor
//-----------------------------------------------------------------------------
CBonusMapsMultiplayerSettingsDialog::~CBonusMapsMultiplayerSettingsDialog()
{
	g_pBonusMapsDialog->m_MultiplayerSettingsDialog = NULL;
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBonusMapsMultiplayerSettingsDialog::OnCommand( const char *command )
{	
	if ( !stricmp ( command, "ApplyOptions" ) )
	{
		char sHostname[64];
		m_pHostnameTextEntry->GetText( sHostname, m_pHostnameTextEntry->GetTextLength() + 1 );
		
		char sPassword[64];
		m_pPasswordTextEntry->GetText( sPassword, m_pPasswordTextEntry->GetTextLength() + 1 );

		const char* pszHostname = sHostname;
		const char* pszPassword = sPassword;
		
		V_strcpy_safe( m_MultiplayerOptions.sHostname, pszHostname );
		V_strcpy_safe( m_MultiplayerOptions.sPassword, pszPassword );
				
		m_MultiplayerOptions.bForceRespawn = m_pForceRespawnCheck->IsSelected();
		m_MultiplayerOptions.bLanServer = m_pLanServerCheck->IsSelected();

		g_pBonusMapsDialog->m_MultiplayerOptions = m_MultiplayerOptions;
...
void CBonusMapsMultiplayerSettingsDialog::OnKeyCodeTyped( ButtonCode_t code )
{
...
static void CC_OpenBonusMapsDialog2(const CCommand &args)
{
	vgui::Panel *parent = (vgui::Panel *)NULL;

	if (!g_pBonusMapsDialog)
		g_pBonusMapsDialog = new CBonusMapsDialog(parent);
	
	if (!g_pBonusMapsDialog)
		return;

	VPANEL GameUiDll = enginevgui->GetPanel(PANEL_GAMEUIDLL);
	g_pBonusMapsDialog->Activate();
	g_pBonusMapsDialog->SetParent(GameUiDll);
	
}

static ConCommand OpenBonusMapsDialog2( "OpenBonusMapsDialog2", CC_OpenBonusMapsDialog2, "Open client sided bonus maps" );

// It was surprisingly infuriating to get these values to actually work
#if 0
static void CC_GetBonusMapMPSettings(const CCommand &args)
{
	if (!g_pBonusMapsDialog)
		return;

	Msg("Hostname: %s\n", g_pBonusMapsDialog->m_MultiplayerOptions.sHostname);
	Msg("password: %s\n", g_pBonusMapsDialog->m_MultiplayerOptions.sPassword);
	Msg("forcerespawn: %i\n", g_pBonusMapsDialog->m_MultiplayerOptions.bForceRespawn);
}

static ConCommand GetBonusMapMPSettings( "GetBonusMapMPSettings", CC_GetBonusMapMPSettings, "Get MP settings" );
#endif