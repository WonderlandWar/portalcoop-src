...
//Debug visualization
ConVar	g_debug_portal_turret_ceiling( "g_debug_portal_turret_ceiling", "0" );
...
#define TURRET_CEILING_DAMAGE_MULTIPLIER 3.0f
#define TURRET_CEILING_BULLET_FORCE_MULTIPLIER 0.4f

//Activities
extern int ACT_CEILING_TURRET_OPEN;
extern int ACT_CEILING_TURRET_CLOSE;
extern int ACT_CEILING_TURRET_OPEN_IDLE;
extern int ACT_CEILING_TURRET_CLOSED_IDLE;
extern int ACT_CEILING_TURRET_FIRE;
extern int ACT_CEILING_TURRET_DRYFIRE;
...
{
	PORTAL_CEILING_TURRET_DISABLED = TURRET_STATE_TOTAL,
	PORTAL_CEILING_TURRET_SHOTAT,
	PORTAL_CEILING_TURRET_DISSOLVED,

	PORTAL_CEILING_TURRET_STATE_TOTAL
};


const char *g_CeilingTalkNames[] = 
{
...
const char *g_PortalCeilingTalkNames[PORTAL_CEILING_TURRET_STATE_TOTAL - TURRET_STATE_TOTAL] =
{
...
const char* GetCeilingTurretTalkName( int iState )
{
	if ( iState < TURRET_STATE_TOTAL )
		return g_CeilingTalkNames[ iState ];
	
	return g_PortalCeilingTalkNames[iState - TURRET_STATE_TOTAL];
}

//
// Portal Ceiling Turret
//

class CNPC_Portal_CeilingTurret : public CAI_BaseNPC
{
	DECLARE_CLASS( CNPC_Portal_CeilingTurret, CAI_BaseNPC );
public:
	
	CNPC_Portal_CeilingTurret( void );
	~CNPC_Portal_CeilingTurret( void );
...
//Datatable
BEGIN_DATADESC( CNPC_Portal_CeilingTurret )
...
IMPLEMENT_SERVERCLASS_ST(CNPC_Portal_CeilingTurret, DT_NPC_Portal_CeilingTurret)
...
LINK_ENTITY_TO_CLASS( npc_portal_turret_ceiling, CNPC_Portal_CeilingTurret );

//-----------------------------------------------------------------------------
// Constructor
//-----------------------------------------------------------------------------
CNPC_Portal_CeilingTurret::CNPC_Portal_CeilingTurret( void )
{
	m_bActive			= false;
	m_pEyeGlow			= NULL;
	m_iAmmoType			= -1;
...
// Purpose: Precache
//-----------------------------------------------------------------------------
void CNPC_Portal_CeilingTurret::Precache( void )
{
	PrecacheModel( CEILING_TURRET_MODEL );	
	PrecacheModel( CEILING_TURRET_GLOW_SPRITE );

	// Activities
	ADD_CUSTOM_ACTIVITY( CNPC_Portal_CeilingTurret, ACT_CEILING_TURRET_OPEN );
	ADD_CUSTOM_ACTIVITY( CNPC_Portal_CeilingTurret, ACT_CEILING_TURRET_CLOSE );
	ADD_CUSTOM_ACTIVITY( CNPC_Portal_CeilingTurret, ACT_CEILING_TURRET_CLOSED_IDLE );
	ADD_CUSTOM_ACTIVITY( CNPC_Portal_CeilingTurret, ACT_CEILING_TURRET_OPEN_IDLE );
	ADD_CUSTOM_ACTIVITY( CNPC_Portal_CeilingTurret, ACT_CEILING_TURRET_FIRE );
	ADD_CUSTOM_ACTIVITY( CNPC_Portal_CeilingTurret, ACT_CEILING_TURRET_DRYFIRE );
	...
	for ( int iTalkScript = 0; iTalkScript < PORTAL_CEILING_TURRET_STATE_TOTAL; ++iTalkScript )
	{
		if ( iTalkScript < TURRET_STATE_TOTAL )
		{
			if ( g_CeilingTalkNames[ iTalkScript ] )
				PrecacheScriptSound( g_CeilingTalkNames[ iTalkScript ] );
			else
				iTalkScript = TURRET_STATE_TOTAL - 1;	// We hit the last script item, so jump to the portal only states
		}
		else
		{
			PrecacheScriptSound( g_PortalCeilingTalkNames[ iTalkScript - TURRET_STATE_TOTAL ] );
		}
	}
...
//-----------------------------------------------------------------------------
void CNPC_Portal_CeilingTurret::Spawn( void )
{ 
	Precache();
	...
	{
		SetThink( &CNPC_Portal_CeilingTurret::AutoSearchThink );
	...
	m_bOutOfAmmo = ( m_spawnflags & SF_CEILING_TURRET_OUT_OF_AMMO ) != 0;
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
int CNPC_Portal_CeilingTurret::OnTakeDamage( const CTakeDamageInfo &inputInfo )
{
	if ( !m_takedamage )
		return 0;
	...
	if (m_iHealth > 0)
	{
		if (m_lifeState == LIFE_ALIVE && (info.GetDamageType() & DMG_BULLET) && !info.GetAttacker()->IsPlayer())
		{
			...
			{
				EmitSound(GetCeilingTurretTalkName(PORTAL_CEILING_TURRET_SHOTAT));
	...
	if ( m_iHealth <= 0 )
	{
		...
		SetThink( &CNPC_Portal_CeilingTurret::DeathThink );
...
//-----------------------------------------------------------------------------
void CNPC_Portal_CeilingTurret::Retire( void )
{
	...
	{	
		...
		else
		{
			//Set our visible state to dormant
			SetEyeState( TURRET_EYE_DISABLED );
			SetThink( &CNPC_Portal_CeilingTurret::SUB_DoNothing );
		}
	}
}
...
//-----------------------------------------------------------------------------
void CNPC_Portal_CeilingTurret::Deploy( void )
{
	...
	if ( IsActivityFinished() )
	{
		...
		m_flPlaybackRate = 0;
		SetThink( &CNPC_Portal_CeilingTurret::SearchThink );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Portal_CeilingTurret::SetLastSightTime()
{
...
//-----------------------------------------------------------------------------
float CNPC_Portal_CeilingTurret::MaxYawSpeed( void )
{
...
//-----------------------------------------------------------------------------
bool CNPC_Portal_CeilingTurret::UpdateFacing( void )
{
	bool  bMoved = false;
	...
	if ( g_debug_portal_turret_ceiling.GetBool() )
	{
...
//-----------------------------------------------------------------------------
bool CNPC_Portal_CeilingTurret::FVisible( CBaseEntity *pEntity, int traceMask, CBaseEntity **ppBlocker )
{
...
//-----------------------------------------------------------------------------
void CNPC_Portal_CeilingTurret::ActiveThink( void )
{
	LaserOn();
	...
	if ( ( bEnemyVisible == false ) || ( m_flDistToEnemy > CEILING_TURRET_RANGE ))
	{
		if ( m_flLastSight )
		{
	...
	{
		//We want to look at the enemy's eyes so we don't jitter
		Vector vEnemyWorldSpaceCenter = GetEnemy()->WorldSpaceCenter();
...
//-----------------------------------------------------------------------------
void CNPC_Portal_CeilingTurret::SearchThink( void )
{
	//Allow descended classes a chance to do something before the think function
	...
	if ( GetEnemy() != NULL )
	{
		...
		m_flLastSight = 0;
		SetThink( &CNPC_Portal_CeilingTurret::ActiveThink );
		SetEyeState( TURRET_EYE_SEE_TARGET );
 	...
 	if ( gpGlobals->curtime > m_flLastSight )
	{
		...
		m_flLastSight = 0;
		SetThink( &CNPC_Portal_CeilingTurret::Retire );
		return;
	}
...
//-----------------------------------------------------------------------------
void CNPC_Portal_CeilingTurret::AutoSearchThink( void )
{
	//Allow descended classes a chance to do something before the think function
	...
	if ( GetEnemy() != NULL )
	{
		SetThink( &CNPC_Portal_CeilingTurret::Deploy );
...
// Purpose: Fire!
//-----------------------------------------------------------------------------
void CNPC_Portal_CeilingTurret::Shoot( const Vector &vecSrc, const Vector &vecDirToEnemy )
{
	if ( m_spawnflags & SF_CEILING_TURRET_OUT_OF_AMMO )
	{
	...
	info.m_flDamageForceScale = ( ( !m_bDamageForce ) ? ( 0.0f ) : ( TURRET_CEILING_BULLET_FORCE_MULTIPLIER ) );
	
	if ( m_iLastState == TURRET_ACTIVE && gpGlobals->curtime > m_fNextTalk )
	{
		EmitSound( GetCeilingTurretTalkName( m_iLastState ) );
...
//-----------------------------------------------------------------------------
bool CNPC_Portal_CeilingTurret::PreThink( turretState_e state )
{
	// Working 2 enums into one integer
	int iNewState = state;
	
	// Need to play these sounds immediately
	if ( m_iLastState != iNewState && ( ( iNewState == TURRET_DEAD && !m_bDelayTippedTalk ) || 
	...
	if ( gpGlobals->curtime > m_fNextTalk && m_iLastState != iNewState )
	{
		...
		const char *pchScriptName = GetCeilingTurretTalkName( m_iLastState );
...
//-----------------------------------------------------------------------------
void CNPC_Portal_CeilingTurret::SetEyeState( eyeState_t state )
{
	...
	case TURRET_EYE_SEEKING_TARGET: //Ping-pongs
		...
		if ( m_bBlinkState )
		{
			...
			m_pEyeGlow->SetScale( 0.3f, 0.1f );
			m_pEyeGlow->SetBrightness( 224, 0.1f );
		}
		else
		{
			//Fade down and scale down
			m_pEyeGlow->SetScale( 0.2f, 0.1f );
			m_pEyeGlow->SetBrightness( 192, 0.1f );
		}
	...
	case TURRET_EYE_DORMANT: //Fade out and scale down
		m_pEyeGlow->SetColor(255, 0, 0);
		m_pEyeGlow->SetScale(0.2f, 0.5f);
		m_pEyeGlow->SetBrightness(192, 0.5f);
		break;
...
float CNPC_Portal_CeilingTurret::GetAttackDamageScale(CBaseEntity *pVictim)
{
	...
	if ( pBCC )
	{
		...
		{
			return TURRET_CEILING_DAMAGE_MULTIPLIER;
		}
	}
...
//-----------------------------------------------------------------------------
void CNPC_Portal_CeilingTurret::Ping( void )
{
...
//-----------------------------------------------------------------------------
void CNPC_Portal_CeilingTurret::Toggle( void )
{
	//Toggle the state
	if ( m_bEnabled )
	{
...
//-----------------------------------------------------------------------------
void CNPC_Portal_CeilingTurret::Enable( void )
{
	m_bEnabled = true;
...
//-----------------------------------------------------------------------------
void CNPC_Portal_CeilingTurret::Disable( void )
{
	m_bEnabled = false;
	m_bAutoStart = false;
...
//-----------------------------------------------------------------------------
void CNPC_Portal_CeilingTurret::InputToggle( inputdata_t &inputdata )
{
	Toggle();
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Portal_CeilingTurret::InputEnable( inputdata_t &inputdata )
{
	Enable();
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Portal_CeilingTurret::InputDisable( inputdata_t &inputdata )
{
	Disable();
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Portal_CeilingTurret::SpinUp( void )
{
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Portal_CeilingTurret::SpinDown( void )
{
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Portal_CeilingTurret::DeathThink( void )
{
	...
	if ( m_lifeState != LIFE_DEAD )
	{
		...
		EmitSound( GetCeilingTurretTalkName( PORTAL_CEILING_TURRET_DISABLED ) );
...
// Input  : height - 
//-----------------------------------------------------------------------------
void CNPC_Portal_CeilingTurret::SetHeight( float height )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_Portal_CeilingTurret::CanBeAnEnemyOf( CBaseEntity *pEnemy )
{
...
void CNPC_Portal_CeilingTurret::LaserOff( void )
{
	m_bLaserOn = false;
}

void CNPC_Portal_CeilingTurret::LaserOn( void )
{
	m_bLaserOn = true;
}