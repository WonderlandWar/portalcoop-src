...
{ // Line 96
    ...
    #endif
		    vTargetDir = vecTestOrg - vecOrigin;
		    VectorNormalize(vTargetDir);
...
//-----------------------------------------------------------------------------
void CInfoPlacementHelper::NotifyPortalEvent( PortalEvent_t nEventType, CProp_Portal *pNotifier )
{ // Line 262
...
// Purpose: Completely restored
//-----------------------------------------------------------------------------
void CInfoPlacementHelper::BindToPortal( CProp_Portal *pPortal )
{ // Line 286
    if ( !pPortal ) // Line 287
        return;
