...
CON_COMMAND(map_transition, "Fires test event")
{
	CBasePlayer *pPlayer = UTIL_GetCommandClient();

	bool bShouldDoCommand = false;

	if (!pPlayer)
		bShouldDoCommand = true;
	else if (pPlayer == UTIL_GetListenServerHost())
		bShouldDoCommand = true;

	if (bShouldDoCommand)
	{

		IGameEvent * event = gameeventmanager->CreateEvent("map_transition", false);
		if (event)
		{
			//event->SetString("hint_name", GetEntityName().ToCStr());
			...
			Msg("Fired event: map_transition ");
		}
	}
