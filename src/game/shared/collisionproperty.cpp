...
void CCollisionProperty::UpdateServerPartitionMask( )
{
	...
	if (IsSolidFlagSet(MOVETYPE_PUSH) || IsSolidFlagSet(MOVETYPE_NONE) || IsSolidFlagSet(MOVETYPE_VPHYSICS) || IsSolidFlagSet(MOVETYPE_NOCLIP))
	{
		mask |= PARTITION_ENGINE_PUSHABLE;
	}
	Assert( mask != 0 );
