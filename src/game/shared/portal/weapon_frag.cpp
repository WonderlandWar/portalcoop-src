...
LINK_ENTITY_TO_CLASS_ALIASED( weapon_frag, WeaponFrag );
...
#ifdef CLIENT_DLL
	//BD 3D
	QAngle qFacing = pPlayer->GetAbsAngles();

	// FIXME: cache this
	Vector vecFacing;
	AngleVectors( qFacing, &vecFacing );

	//BD 2D
	vecFacing.z = 0;
	vecFacing.AsVector2D().NormalizeInPlace();
#else
