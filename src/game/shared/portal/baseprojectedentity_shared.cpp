...
// 
//--------------------------------------------------------------------------------------------------
bool CBaseProjectedEntity::IsHittingPortal( Vector* pOutOrigin, QAngle* pOutAngles, CProp_Portal** pOutPortal )
{
#if defined( GAME_DLL )
	...
	CProp_Portal* pHitPortal = UTIL_Portal_FirstAlongRay( ray, flPortalTraceFraction );
...
// 
//--------------------------------------------------------------------------------------------------
void CBaseProjectedEntity::RecursiveProjection( bool bShouldSpawn, CBaseProjector *pParentProjector, CProp_Portal *pExitPortal, const Vector &vProjectOrigin, const QAngle &qProjectAngles, int iRemainingProjections )
{
#if defined( CLIENT_DLL )
	...
	if ( iRemainingProjections > 1 )
	{
		...
		{			
			CProp_Portal *pNewExitPortal = pHitPortal->m_hLinkedPortal.Get();
...
#endif
			{
				m_hChildSegment.Get()->RecursiveProjection( bCreateNew, pParentProjector, pNewExitPortal, vNewProjectedEntityOrigin, qNewProjectedEntityAngles, iRemainingProjections - 1 );
			}
		}
...
#endif
		{
			pChild->RecursiveProjection( bCreateNew, (CBaseProjector *)GetOwnerEntity(), pHitPortal->m_hLinkedPortal.Get(), vNewPosition, qNewAngles,
				m_iMaxRemainingRecursions - 1 );
		}
	}
...
//--------------------------------------------------------------------------------------------------
bool CBaseProjectedEntity::DidRedirectionPortalMove( CProp_Portal* pPortal )
{
	if ( !pPortal )
		return true;
	...
	// close portal moved
	if ( VectorsAreEqual( pChild->m_vecSourcePortalRemoteCenter, pPortal->GetAbsOrigin() ) == false )
	{
		//EASY_DIFFPRINT( pChild, "CBaseProjectedEntity::DidRedirectionPortalMove() hit portal moved" );
		return true;
	}
	// close portal rotated
	if ( QAnglesAreEqual( pChild->m_vecSourcePortalRemoteAngle, pPortal->GetAbsAngles() ) == false )
	{
		return true;
	}
	...
	// remote portal moved
	if ( VectorsAreEqual( pChild->m_vecSourcePortalCenter, pPortal->m_hLinkedPortal->GetAbsOrigin() ) == false )
	{
		//EASY_DIFFPRINT( pChild, "CBaseProjectedEntity::DidRedirectionPortalMove() remote portal moved" );
		return true;
	}
	// remote portal rotated
	if ( QAnglesAreEqual( pChild->m_vecSourcePortalAngle, pPortal->m_hLinkedPortal->GetAbsAngles() ) == false )
	{
		return true;
	}
...
// 
//--------------------------------------------------------------------------------------------------
void CBaseProjectedEntity::SetHitPortal( CProp_Portal* pPortal )
{
...
CProp_Portal* CBaseProjectedEntity::GetHitPortal( void )
{
...
// 
//--------------------------------------------------------------------------------------------------
void CBaseProjectedEntity::SetSourcePortal( CProp_Portal* pPortal )
{
...
CProp_Portal* CBaseProjectedEntity::GetSourcePortal( void )
{
...
#endif
		{
			m_hChildSegment.Get()->RecursiveProjection( bCreateNew, (CBaseProjector *)GetOwnerEntity(), NULL, vNewProjectedEntityOrigin, qNewProjectedEntityAngles, m_iMaxRemainingRecursions - 1 );
		}
	}
#if defined( GAME_DLL )
	else if ( m_hChildSegment.Get() != NULL )
