...
LINK_ENTITY_TO_CLASS_ALIASED( weapon_rpg, WeaponRPG );
...
void CWeaponRPG::PrimaryAttack( void )
{
	...
	pMissile->SetDamage( GetPortalWpnData().m_iPlayerDamage );
...
void CWeaponRPG::DrawEffects( void )
{
	...
	g_bBulletPortalTrace = true;
	Ray_t rayLaser;
	rayLaser.Init( vecAttachment, vecAttachment + ( vecDir * RPG_LASER_BEAM_LENGTH ) );
	UTIL_Portal_TraceRay( rayLaser, MASK_SHOT, GetOwner(), COLLISION_GROUP_NONE, &tr );
	g_bBulletPortalTrace = false;
#else
...
LINK_ENTITY_TO_CLASS_ALIASED( env_laserdot, LaserDot );
...
int CLaserDot::DrawModel( int flags )
{
	...
	g_bBulletPortalTrace = true;
	Ray_t rayLaser;
	rayLaser.Init( vecAttachment, vecAttachment + ( vecDir * MAX_TRACE_LENGTH ) );
	UTIL_Portal_TraceRay( rayLaser, MASK_SHOT, pOwner, COLLISION_GROUP_NONE, &tr );
	g_bBulletPortalTrace = false;
#else
